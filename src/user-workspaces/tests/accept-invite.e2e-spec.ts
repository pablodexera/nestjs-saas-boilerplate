...
describe('[UserWorkspacesE2E] POST /user-workspaces/:workspaceId/members/accept', () => {
  ...
  beforeEach(async () => {
    ...
    const uniqueSlug = `e2e-accept-invite-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
    const res = await request(app.getHttpServer())
      .post('/workspaces')
      .set('Authorization', `Bearer ${admin.token}`)
      .send({ name: 'Accept Invite Workspace', slug: uniqueSlug, owner_id: admin.id });
  ...
  it('allows invited user to accept invite', async () => {
    const res = await request(app.getHttpServer())
      .post(`/user-workspaces/${workspaceId}/members/accept`)
      .set('Authorization', `Bearer ${member.token}`)
      .expect(200);
    expect(res.body.status).toBe('active');
  });

  it('returns 404 for already active membership', async () => {
    // Accept once
